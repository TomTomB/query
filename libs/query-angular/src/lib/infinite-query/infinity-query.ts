...
> {
  ...
  private readonly _itemsPerPage$ = new BehaviorSubject<number>(
    this._config.limitParam?.value ?? 10
  );
  ...
  private _prepareArgs(
    ...
    const limitParamLocation = this._getPageParamLocation(
      config?.limitParam?.location
    );
  ...
  private _getPageParamLocation(
    location: InfinityQueryParamLocation | undefined
  ) {
