...
export class BearerAuthProvider implements AuthProvider {
  ...
  private _setupRefresh(config: BearerRefreshConfig) {
    ...
    if (expiresIn === undefined) {
      throw new Error(
        `Bearer token does not contain an '${expiresInPropertyName}' property`
      );
    }

    const remainingTime =
      new Date(expiresIn * 1000).getTime() - fiveMinutes - new Date().getTime();

    const strategy =
      config.strategy ?? AuthBearerRefreshStrategy.BeforeExpiration;

    switch (strategy) {
      case AuthBearerRefreshStrategy.BeforeExpiration:
        this._refreshTimerSubscription = timer(remainingTime).subscribe(() =>
  ...
  private async _refresh(config: BearerRefreshConfig) {
    ...
    const body = bodyAdapter
      ? bodyAdapter(this._refreshToken)
    ...
    try {
      ...
      const tokens = responseAdapter
        ? responseAdapter(result.data)
        : (result.data as TokenResponse);
