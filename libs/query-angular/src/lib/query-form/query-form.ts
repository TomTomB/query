...
export class QueryField<T> {
  ...
  private _setupChangesObservable() {
    ...
    return concat(
      obs.pipe(take(1)),
      this.control.valueChanges.pipe(debounceTime(this.data.debounce))
    );
  }
}
...
export class QueryForm<T extends Record<string, QueryField<any>>> {
  ...
  observe(options?: { syncViaUrlQueryParams?: boolean }) {
    return combineLatest(
      Object.values(this._fields).map((field) => field.changes$)
    ).pipe(
  ...
  setFormValueFromUrlQueryParams() {
    if (!this._activatedRoute) {
      throw new Error(
        'Cannot set form value from url query params without ActivatedRoute'
      );
    }
    ...
    for (const [key, field] of Object.entries(this._fields)) {
      ...
      const deserializedValue =
        field.data.queryParamTransformFn?.(value) ?? value;
  ...
  updateFormOnUrlQueryParamsChange() {
    if (!this._activatedRoute) {
      throw new Error(
        'Cannot update form on url query params change without ActivatedRoute'
      );
    }
  ...
  private _syncViaUrlQueryParams(values: Record<string, unknown>) {
    ...
    const queryParams = structuredClone(
      this._activatedRoute.snapshot.queryParams
    );

    for (const [key, value] of Object.entries(values)) {
      if (
        this._isDefaultValue(key, value) ||
        this._fields[key].data.appendToUrl === false
      ) {
